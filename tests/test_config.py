...
def test_load_config_reads_committed_example():
    ...
    assert cfg.window_end == "07:20"


def test_retired_keys_are_ignored(tmp_path):
    # show_count/show_cheapest no longer exist — a config still carrying them
    # (as the other tests here do) must load rather than blow up.
    f = tmp_path / "c.local"
    f.write_text('{"origin_nlc":"1","origin_name":"A","destination_nlc":"2",'
                 '"destination_name":"B","travel_days":["Mon"],"window_start":"05:45",'
                 '"window_end":"08:00","show_count":5,"show_cheapest":2,'
                 '"storage_path":"~/x/prices.json","request_pause_seconds":1.0}')
    cfg = load_config(f)
    assert not hasattr(cfg, "show_count")

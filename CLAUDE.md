...
- **Transient 5xx get exactly one retry** (`_RETRY_STATUSES` in `client.py`), spaced by the same per-call pause — SouthEastern's backend sometimes returns a 500 "InternalError" that succeeds on the next try. One retry only; never loop on a failing server. A genuine failure (5xx after the retry, or a transport fault) raises `TrainApiError`, which `lookup_day` turns into a "left this day unchanged" result so a server hiccup never wipes saved data or masquerades as an empty window.
- Budget: a full week lookup is roughly 27 spaced requests (3 days × (1 plan + ~8 detail fetches)). The plan carries no departure times, so we must fetch the detail of every journey it returns to sort by departure. Every journey in the window is then kept — the detail calls have already been paid for, so trimming would save nothing and would hide later trains in the window that may be cheaper than the early ones. `view` decides how many to display (`_TRAINS_PER_DAY`). A window returns only a handful of journeys, so this stays modest — but it is one detail call per journey, not per displayed train.
